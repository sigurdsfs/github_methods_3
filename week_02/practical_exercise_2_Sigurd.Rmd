...
politeness_F1 <- politeness %>% 
  filter(subject == "F1")
m1 <- lm(f0mn ~ scenario, data = politeness_F1, refresh = 0)
m2 <- lm(f0mn ~ as.integer(scenario), data = politeness_F1, refresh = 0)
```
...
```{r}
m3.4 <- lmer(f0mn ~ (1|scenario) + (1|subject), data = politeness, REML = F)
```
...
solved with a simple yuen trim of outliers. The first model 3.1 holds the assumptions
...
ggplot(politeness, aes(x = scenario, y = f0mn)) + geom_point() + facet_wrap(subject~scenario)
```
...
```{r}
#reidual variance 
c(RS_5.2 = summary(m5.2)$sigma^2,
RS_5.1 = summary(m5.1)$sigma^2,
RS_3.4 = summary(m3.4)$sigma^2)
...
(See imaginiry study). However the interaction between gender:attitude doesn't add
anything explaining to the model. 
